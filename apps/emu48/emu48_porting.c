...
#define vPortEnterCritical(x) 
#define vPortExitCritical(x)
uint32_t emu_add_timer(uint32_t period_ms, LPTIMECALLBACK cb, uint32_t userData, bool oneshot)
{
    ...
    if(emu_timers)
    {
        ...
        t = malloc(sizeof(emu48_emu_timer_t));
    }else{
        t = emu_timers = malloc(sizeof(emu48_emu_timer_t));
        t_prev = NULL;
    }
...
static uint8_t screen_line_buf[260];
...
void  __attribute__((target("arm")))  emu48_disp_update_task(void *_)
{
    ...
    for(;;)
    {
        if(need_Refresh)
        {
            for(uint16_t y = 0; y < SCREENHEIGHT; y++)
            {
                ...
                {
                    screen_line_buf[tx++] = pbyLcd[144 * y + x];
                    screen_line_buf[tx++] = pbyLcd[144 * y + x];
                }
                llapi_disp_put_hline(2*y, screen_line_buf);
                llapi_disp_put_hline(1 + 2*y, screen_line_buf);
    ...
    for(;;)
    {
        if(need_Refresh)
        {
                ...
                for(int y = 0; y < 127; y++)
                {
                    llapi_disp_put_hline(y, &full_screen_buf[y * 256]);
                }
...
LPBYTE FASTPTR(DWORD d)
{
	...
	if ((lpbyPage = RMap[d>>12]) != NULL)	// page valid
	{
        ...
        if((uint32_t)lpbyPage >= 0x30000000)
        {
            //if((uint32_t)lpbyPage < 0x70000000 + dwRomSize * 2)
            {

                //memcpy(pbyCODE, (void *)((uint32_t)lpbyPage - 0x30000000 + (uint32_t)emu_rom_addr) ,128);
                //return pbyCODE;

                uint32_t acc_addr = ((uint32_t)lpbyPage - 0x30000000) / 2 + (uint32_t)emu_rom_addr;
...
/*
    ...
    */
    ...
    if(ret < 0)
    {
        llapi_disp_put_kstr(0,0,"ROM.39G Not Found!", (0xFF));
        llapi_disp_put_kstr(0,1*16,"Long-Press [ON] to exit", (0xFF));
        goto b_exit;
    }
    ...
    {
        llapi_mumap(ret);
        llapi_disp_put_kstr(0,0,"ROM.39G is not unpacked!", (0xFF));
        llapi_disp_put_kstr(0,1*16,"Long-Press [ON] to exit", (0xFF));
        goto b_exit;
    }
...
*/
    ...
    pbyLcd = malloc(36 * 4 * SCREENHEIGHT);
    ...
    assert(f);
    llapi_fs_open(f, "/emu48_portmem", O_CREAT | O_WRONLY | O_TRUNC);
    llapi_fs_seek(f, (2*128*2048) - 1, SEEK_SET);
    llapi_fs_write(f, "0", 1);
    llapi_fs_sync(f);
    llapi_fs_close(f);

    mmap_info m0;
    m0.map_to = 0x32000000;
    m0.offset = 0;
    m0.path = "/emu48_portmem";
    m0.size = 0;
    m0.writable = true;
    m0.writeback = false;
	...
	if (Chipset.type == 'E')				// HP39/40G
	{
		...
		//Chipset.Port0Size = 64;
		//Chipset.Port1Size = 0;
		//Chipset.Port2Size = 16;
    ...
    Chipset.Port0 = (void *)(0x32000000);
    Chipset.Port2 = (void *)(0x32000000 + Chipset.Port0Size*2048);
...
*/
    ...
    llapi_thread_create(emu_thread, malloc(1532), 1532, NULL);
    llapi_thread_create(emu48_disp_update_task, malloc(1000), 1000, NULL);
    llapi_thread_create(emu_timer_thread, malloc(1000), 1000, NULL);
...
VOID GetLocalTime(struct SYSTEMTIME *lpSystemTime) {
    if(lpSystemTime) {
        ...
        lpSystemTime->wSecond = llapi_rtc_get_s();
        lpSystemTime->wMilliseconds = llapi_get_tick_ms() ;
        /*
...
{
    time_t t = llapi_get_tick_ms();//;time(NULL);
    return (DWORD)t;
    //return (DWORD)(t * 1000);
}
